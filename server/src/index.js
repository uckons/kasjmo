...
const distCandidates = [process.env.CLIENT_DIST_PATH, path.resolve(__dirname, '../../client/dist'), path.resolve(process.cwd(), '../client/dist'), path.resolve(process.cwd(), 'client/dist')].filter(Boolean); const clientDistPath = distCandidates.find((p) => fs.existsSync(p));
if (clientDistPath) { const assetDir = path.join(clientDistPath, 'assets'); console.log(`Serving frontend from: ${clientDistPath}`); app.get('/assets/:file', (req, res, next) => { const requested = path.join(assetDir, req.params.file); if (fs.existsSync(requested)) return res.sendFile(requested); if (!fs.existsSync(assetDir)) return next(); const files = fs.readdirSync(assetDir); if (req.params.file.endsWith('.js')) { const fallbackJs = files.find((f) => f === 'app.js') || files.find((f) => /^index-.*\.js$/.test(f)); if (fallbackJs) return res.sendFile(path.join(assetDir, fallbackJs)); } if (req.params.file.endsWith('.css')) { const fallbackCss = files.find((f) => f === 'app.css') || files.find((f) => /^index-.*\.css$/.test(f)); if (fallbackCss) return res.sendFile(path.join(assetDir, fallbackCss)); } return next(); }); app.use(express.static(clientDistPath, { setHeaders: (res, filePath) => { if (filePath.endsWith('index.html')) res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate'); else if (filePath.includes(`${path.sep}assets${path.sep}`)) res.setHeader('Cache-Control', 'public, max-age=31536000, immutable'); } })); app.get('*', (req, res, next) => { if (req.path.startsWith('/api/')) return next(); res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate'); res.sendFile(path.join(clientDistPath, 'index.html')); }); } else { console.warn(`Frontend dist folder not found. Tried: ${distCandidates.join(', ')}`); }
