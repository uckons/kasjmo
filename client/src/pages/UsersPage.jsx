...
export default function UsersPage() {
  ...
  async function resetPasswordByAdmin(user) {
    const newPassword = passwordDraft[user.id] || '';
    if (!newPassword || newPassword.length < 8) {
      setMessage('Password baru minimal 8 karakter');
      return;
    }
    try {
      await http.post(`/users/${user.id}/reset-password`, { newPassword });
      setPasswordDraft((prev) => ({ ...prev, [user.id]: '' }));
      setMessage(`Password ${user.full_name} berhasil direset`);
    } catch (err) {
      setMessage(err.response?.data?.message || 'Gagal reset password user');
    }
  }
