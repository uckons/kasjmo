...
function Badge({ status }) {
  const classes = { approved: 'bg-emerald-100 text-emerald-700', pending_approval: 'bg-amber-100 text-amber-700', rejected: 'bg-rose-100 text-rose-700', draft: 'bg-slate-100 text-slate-700' };
...
export default function TransactionsPage() {
  ...
  async function load() {
    ...
    setItems(txData);
    setCategories(catData.filter((x) => x.is_active));
  }

  useEffect(() => { load(); }, []);
  const pendingApprovals = useMemo(() => items.filter((item) => item.cash_type === 'kas_besar' && item.flow === 'expense' && item.status === 'pending_approval'), [items]);
  ...
  async function submit(e) {
    ...
    try {
      ...
      load();
    } catch (err) { setMessage(err.response?.data?.message || 'Gagal membuat transaksi'); }
  }

  function startEdit(item) {
    setEditId(item.id);
    setEditForm({
      cashType: item.cash_type,
      flow: item.flow,
